...
import pisama_auto
from pisama_auto import _tracer
from pisama_auto.patches import _patched, patch, patch_all
from pisama_auto.patches.anthropic_patch import _traced_stream, _TracedStream
...
def test_init_without_credentials_is_idempotent():
    ...
    try:
        pisama_auto.init(service_name="integration-test", auto_patch=True)
        first_tracer = _tracer._tracer
        pisama_auto.init(service_name="ignored", auto_patch=True)
...
def test_patch_registry_handles_absent_and_broken_integrations(monkeypatch):
    from pisama_auto import patches
